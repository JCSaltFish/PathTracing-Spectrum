...
void Previewer::SetTemperatureTextureForElement(int objId, int elementId, const std::string& file)
{
    if (objId >= mLoadedObjects.size())
        return;
    if (elementId >= mLoadedObjects[objId].elements.size())
    ...
    GLuint& tex = mLoadedObjects[objId].elements[elementId].temperatureTexId;
    if (tex != -1)
    {
        glDeleteTextures(1, &tex);
        tex = -1;
