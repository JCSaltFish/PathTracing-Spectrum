...
void CUDASetWavelength(float v)
{
	gpuErrchk(cudaMemcpyToSymbol(wavelength, &v, sizeof(unsigned)));
}

void CUDASetSky(float sky)
{
	gpuErrchk(cudaMemcpyToSymbol(waveSky, &sky, sizeof(unsigned)));
}
...
__device__ float BBP(float temperature, float wavelength)
{
	float c = 299792458.0f;
	float k = 138064852e-31;
	float h = 2.0f * PI * 105457180e-42;
...
__device__ float GetEmissivity(float matEmiss, float temperature)
{
	return BBP(temperature + 273.15f, wavelength) * matEmiss;
}

__device__ float Trace(glm::vec3 ro, glm::vec3 rd, int& depth, bool& inside, curandState_t& state)
{
	float d = 0.0f;
	Triangle t;
	if (Hit(ro, rd, t, d))
	{
		...
		if (depth < maxDepth * 2)
		{
			...
			if (depth >= maxDepth)
			{
				if (glm::abs(curand_uniform(&state)) > prob)
			...
			float emissivity = mat.emissivity;
			float reflectivity = mat.reflectivity;
			if (mat.temperatureTexId != -1)
			{
				// TODO map temperature value here
				float temperature = CUDATex2D(textures[mat.temperatureTexId], uv).x;
				emissivity = GetEmissivity(t.spectrumMatEmiss, temperature);
			}

			return emissivity + Trace(p, reflectDir, depth, inside, state) * reflectivity;
		}
	}
...
__global__ void RenderPixel(float* img)
{
	...
	int depth = 0;
	bool inside = false;
	float res = Trace(camPos, rayDir, depth, inside, localState);

	// Draw
	glm::vec3 color = glm::vec3(res);
